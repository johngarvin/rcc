		...
		DGraph::DGraphEdgeDirection alongFlow)
{
	...
	do {
		...
		iterCnt++;
		if (debug) {
			std::cout << "DFAVis DGraphIterativeDFP debug 1" << std::endl;
		...
		for (; nodeIterPtr->isValid(); ++(*nodeIterPtr)) {
			...
			numNodeAccess++;
			if (debug) {
				// Have it pause 3 seconds, imitating the actual response for DFAVis
				std::cout << "DFAVis DGraphIterativeDFP debug 2" << std::endl;
	...
	if (debug) {
		std::cout << "DFAVis DGraphIterativeDFP debug 3" << std::endl;
		...
		DFPImplement algorithm)
{
	...
	while (!wlist->isEmpty()) {
		...
		if (debug) {
			std::cout << "DFAVis DGraphIterativeDFP debug 4" << std::endl;
			std::cout << "node: Id = " << node->getId();
			std::cout << ", num_outgoing = " << node->num_outgoing();
			std::cout << ", OA_ptr::dump = ";
			node.dump(std::cout);
		...
		if (changed) {
			if (debug) {
				std::cout << "DFAVis DGraphIterativeDFP debug 5" << std::endl;
			...
			for (; neighIter->isValid(); ++(*neighIter)) {
				...
				wlist->add(n);
				if (debug) {
					std::cout << "DFAVis DGraphIterativeDFP debug 6" << std::endl;
					std::cout << n->getId() << " ";
				}
			}
		}
