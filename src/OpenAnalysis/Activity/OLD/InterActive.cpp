...
{
    OA_ptr<OA::DataFlow::DataFlowSetImplIterator<int> > retval;
  ...
  sOutBuild->objStart("InterActive"); {
    ...
    for (; procIterPtr->isValid(); (*procIterPtr)++) {
      ...
      sOutBuild->fieldStart(oss.str()); {
        ...
        sOutBuild->objStart("ActiveTagSet"); {
          sOutBuild->listStart(); {
            OA_ptr<OA::DataFlow::DataFlowSetImplIterator<int> > tagIterPtr;
            tagIterPtr = getActiveTagsIterator(proc);
            for ( ; tagIterPtr->isValid(); (*tagIterPtr)++ ) {
              int tag = tagIterPtr->current();
