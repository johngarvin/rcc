...
/// For backward flow, this is a return edge
...
/// For backward flow, this is a call edge
							       ...
							       ProcHandle caller)
{
  ...
  assert(is_call(e));
  FuncInfo * callee_fi = getProperty(FuncInfo, callee);
  assert(Rf_length(call_args(e)) == callee_fi->get_num_args());
  for(int i = 1; i != callee_fi->get_num_args(); i++) {
    if (in->lookup(fact->make_arg_var_ref())) {
      propagate_rhs(in, &returned_predicate, true, call_nth_arg(i));
      // TODO: should be returned(call) instead of true
      // but how to get returned(call) if not a name?
    }
  }
}
