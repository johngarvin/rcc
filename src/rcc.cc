...
static Expression bogus_exp = Expression("BOGUS", TRUE, FALSE, "");
...
Expression SubexpBuffer::op_exp(SEXP e, string rho) {
  ...
  case SYMSXP:
    if (e == R_MissingArg) {
      return Expression("R_MissingArg", TRUE, FALSE, "");
    } else {
  ...
  case CLOSXP:
    ...
    if (rho == "R_GlobalEnv" && !formals.is_dep && !body.is_dep) {
      ...
      out = Expression(v, FALSE, FALSE, unp(v));
    } else {
...
Expression SubexpBuffer::op_arglist(SEXP e, string rho) {
  ...
  SEXP arg;
  int len = Rf_length(e);
  if (len == 0) return Expression("R_NilValue", TRUE, FALSE, "");
...
Expression SubexpBuffer::op_literal(SEXP e, string rho) {
  ...
  case VECSXP:
    ...
    break;
  case SYMSXP:
    return Expression(make_symbol(e), TRUE, TRUE, "");
    break;
  case LISTSXP:
    return op_list(e, rho);
    break;
